...
namespace StorybrewScripts
{
    ...
    public delegate Vector2 EquationFunction(Vector2 currentPosition, double currentTime, double t);
                    ...
                    do
                    {
                        ...
                        if (currentEffect.Value != null && currentEffect.Value.effektType == EffectType.RenderPlayFieldFrom)
                        {
                            if (easedProgress < currentEffect.Value.value)
                            {
                                ...
                                Vector2 originPositionLocal = column.origin.getCurrentPosition(currentTime + localIterationRate);
                                Vector2 receptorPositionLocal = column.receptor.getCurrentPosition(currentTime + localIterationRate);
                        ...
                        newPosition = equation(newPosition, currentTime + localIterationRate, easedProgress);
                        ...
                        do
                        {
                            ...
                            if (currentEffect.Value != null && currentEffect.Value.effektType == EffectType.RenderPlayFieldFrom)
                            {
                                if (easedProgress < currentEffect.Value.value)
                                {
                                    ...
                                    Vector2 originPositionLocal = column.origin.getCurrentPosition(sliderCurrentTime + sliderIterationLenght);
                                    Vector2 receptorPositionLocal = column.receptor.getCurrentPosition(sliderCurrentTime + sliderIterationLenght);
                                    Vector2 newPositionLocal = Vector2.Lerp(originPositionLocal, receptorPositionLocal, easedProgress);
